...
export function QrScannerModal({ onClose }) {
  ...
  const [estado, setEstado]       = useState('scanning') // scanning | found | not_found | success | camera_error
    ...
    navigator.mediaDevices
      ...
      .then(stream => {
        ...
        if (video) {
          ...
          video.play().then(() => { if (active) animRef.current = requestAnimationFrame(tick) }).catch(() => {})
        }
      })
  ...
  const registarEntrada = async () => {
    setARegistar(true)
    try {
      await equipamentosApi.registarEntrada(equipamento.movimentoAberto.id)
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      {estado === 'found' && equipamento && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 gap-5">
          <div className="w-full max-w-sm rounded-2xl p-5 bg-white/8 border border-white/12 backdrop-blur-sm">
            ...
            {equipamento.movimentoAberto?.supa_eventos?.evento && (
              <p className="text-xs text-white/35 mt-2">Evento: {equipamento.movimentoAberto.supa_eventos.evento}</p>
            )}
          </div>
          ...
          {equipamento.movimentoAberto ? (
            <button
              ...
              disabled={a_registar}
              className="w-full max-w-sm py-3.5 rounded-2xl bg-blue-600 text-white font-semibold flex items-center justify-center gap-2 active:opacity-80 disabled:opacity-50"
            >
              <LogIn size={16} />
            ...
            <button
              ...
              disabled={a_registar}
              className="w-full max-w-sm py-3.5 rounded-2xl bg-amber-500 text-white font-semibold flex items-center justify-center gap-2 active:opacity-80 disabled:opacity-50"
            >
              <LogOut size={16} />
          ...
          <button onClick={retry} className="text-xs text-white/40 hover:text-white/70">
            Escanear outro
          </button>
        </div>
      )}
      ...
      {estado === 'not_found' && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 gap-4">
          ...
          <button onClick={retry} className="px-6 py-2.5 rounded-xl bg-white/10 text-white text-sm font-medium hover:bg-white/15">
            Tentar de novo
          </button>
      ...
      {estado === 'camera_error' && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 gap-4">
          ...
          <p className="text-white/70 text-sm text-center">
            Sem acesso à câmara.<br />Verifica as permissões do browser.
          </p>
