...
export const colaboradorApi = {
  ...
  async eventosDoTecnico(tecnicoId) {
    ...
    const COLS = 'id,evento,tipo,status,fase,assinatura_lmd_at,assinatura_in_at,assinatura_out_at,assinatura_fim_evento_at,recorrente,com_carro,tecnico_id,todos_tecnicos,contacto_pelo_evento,morada,dia_instalacao,hora_instalacao,data_evento,hora_inicio,hora_fim,notas_operacionais,Equipamentos,notas_colaborador,espaco_id,responsavel,data_preparacao,notas_preparacao,rider_url,fotos_urls,espacos(nome,logo_url)'
    const { data, error } = await supabase
      .from('supa_eventos')
      .select(COLS)
  ...
  async eventosDoTecnicoMes(tecnicoId, dataInicio, dataFim) {
    ...
    const COLS = 'id,evento,tipo,status,com_carro,tecnico_id,contacto_pelo_evento,morada,dia_instalacao,hora_instalacao,data_evento,hora_inicio,hora_fim,notas_operacionais,Equipamentos,notas_colaborador,espaco_id,responsavel,rider_url,fotos_urls,espacos(nome)'
    const { data, error } = await supabase
      .from('supa_eventos')
      .select(COLS)
