...
export const supaEventosApi = {
  ...
  _payload(dados) {
    const COLS = [
      ...
      'proposta_notas_tecnicas', 'proposta_notas_proposta', 'com_carro', 'proposta_linhas', 'tecnicos_externos',
    ]
  ...
  async criar(dados) {
    const { data, error } = await supabase
      ...
      .select('id, evento, data_evento, hora_inicio, hora_fim, hora_instalacao, dia_instalacao, status, espaco_id, tecnico_id, tecnico2_id, tipo, notas_operacionais, Equipamentos, contacto_pelo_evento, morada, artista_id, xclusive, rider_url, fotos_urls, data_preparacao, hora_preparacao, notas_preparacao, proposta_linhas, tecnicos_externos, proposta_notas_tecnicas, proposta_notas_proposta')
      .single()
    if (error) throw error
